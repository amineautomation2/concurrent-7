...
def get_total_funds_mf(headers) -> dict:
    ...
    data_inc = {"query": "inc", "nextId": 0, "instrumentTypes": "FUND"}
    headers.update(
        {
            "referer": "https://www.ii.co.uk/funds",
        }
    )
    ...
    return {"acc": total_acc, "inc": total_inc}
...
def funds_mf(headers: dict, total: int, type: str) -> list[dict]:
    ...
    for p in range(0, total, 3):
        ...
        query = {"query": type, "nextId": p, "instrumentTypes": "FUND"}
        response = fetch_with_backoff(
            f"https://api-prod.ii.co.uk/api/1/faceted-instrument-search-results?query={type}&nextId={p}&instrumentTypes=FUND",
            headers=headers,
            data=query,
        )
        if response:
